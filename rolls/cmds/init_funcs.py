...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if updated_target:
        print(
            f"To change the HDD destination addresses, edit the `rolls_target_address` entries in"
...
def rolls_init(root_path: Path, *, should_check_keys: bool = True, fix_ssl_permissions: bool = False):
    ...
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if HDDCOIN_ROOT is set, or if user has run rolls init twice
        # before a new update.
